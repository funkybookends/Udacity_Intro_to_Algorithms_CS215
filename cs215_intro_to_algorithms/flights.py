...
class Heap():
    """
        Class to keep data in a heap
        Can also keep data associated with an item in the heap
    """
    ...
    def __init__(self,*a):
        #Accepts a list of tuples as (name, data, value)
        self.heap = []
        if a:
            for name,data,value in a:
                self.add(name,data,value)

    def add(self, name = "", data = {}, value = 0):
        #'Add a new name or update the priority of an existing name'
        if name == "": name = next(self.counter)
        if name in self.entry_finder:   #looks for the name in the dict
            self.remove(name)           #performs self.remove it it is in the dict
        count = next(self.counter)      #gives the item a unique couter
        entry = [value, count, data, name] #creatys the entry informationp

        self.entry_finder[name] = entry #adds the entry to the dict
        heappush(self.heap, entry)      #adds the entry to the heap


    def remove(self,name):
        #'Mark an existing name as REMOVED.  Raise KeyError if not found.'
        entry = self.entry_finder.pop(name)
        entry[-1] = self.REMOVED

    def pop(self):
        #'Remove and return the lowest priority name. Raise KeyError if empty.'
        while self.heap:
            value, count, data, name = heappop(self.heap)
    ...
    def bump_up(self,name,data,value):
        #takes in an entry, and improves it if it's better, else leaves it alone
        #if it does not exist, it adds it
        if name in self.entry_finder: #check if it exsists
            if value>=self.entry_finder[name][0]:
                return #if it does and it's worse, don't update
        self.add(name = name, data = data, value = value) #add

    def empty(self):
        #checks if the heap is really empty
        for value,count,data,name in self.heap:
            if name != self.REMOVED: return False
        return True

def to_mins(s): # takes "13:45" and turns it into 13*60+45
    return int(s[0:2])*60 + int(s[3:])

def create_flight_graph(lst):
    """takes in flights as a tuple and sorts them into two dictionarys,
        one where the numbers can be found by the origin
        and one where the keys are the flight numbers"""
    by_origin = {}
    by_number = {}
    for number, origin, destination, departure, arrival, cost in lst:
        if origin not in by_origin: by_origin[origin] = []
        by_origin[origin].append(number)
        by_number[number] = {"origin":origin, "destination" : destination,
              "arrival": arrival, "departure": departure, "cost" : cost}

    return by_origin, by_number


def find_best_flights(flights, origin, destination):
    """Finds the best flight path between origin and destination

        firstly they are sorted by cost,
        secondly they are sorted by flight time
    """
    O,N = create_flight_graph(flights)
    possible_paths = Heap()
    open_list = []

    for fid in O[origin]: ##fid is the flight_id is the flight number
        open_list.append([fid]) ##appends all flights from the origin to the initial list

    def connectable(fid1,fid2):
        """takes two flight dictionarys and confirms that the second can be caught after the first"""
        return to_mins(N[fid1]["arrival"]) < to_mins(N[fid2]["departure"])
    ...
    def duration(path):
        """returns the duration of the path, from the first departure to the last arrival"""
        return to_mins(N[path[-1]]["arrival"]) - to_mins(N[path[0]]["departure"])

    def val(path):
        """calculates the value of the path
            it is stored as the cost in integers and the flight time in 10000th's
        """
        return 0.0+cost(path)+(0.0+duration(path)/10000.0)


    ## Perform the search through all flights
    ## depth first
    while open_list:
        path = open_list.pop() #take out the last path
        if N[path[-1]]["destination"] == destination: #if it goes to the destination,
            v = val(path) #determine the value i.e. cost + flight time
            possible_paths.add(data=path, value = v) ## add it to the heap of possible paths
        else:
            location = N[path[-1]]["destination"] #find the airport we are in
            for fid in O[location]: #for all the flights in from there
                if connectable(path[-1],fid): #if the flight can be taken
                    open_list.append(path+[fid]) #add the flight to the path, and then add it to the list


    if possible_paths.empty(): return None #if there were no paths, return None
    value, name, path = possible_paths.pop() #get the best path + data from the heap
